...
function getActiveNavbar() {
  ...
  const matchingNavbars = [
    ...document.querySelectorAll(`.navbar[data-nav="${activeNav}"]`)
  ];
...
function initNavbarOverlapScroll(navbar) {
  ...
  function updateMeasurements() {
    ...
    tabbarHeight = getHeight(tabbar);
    navHeight = getHeight(nav);
    fullHeight = tabbarHeight + navHeight;
  }

  function syncImmediateState() {
    updateMeasurements();
    ...
    if (isCollapsed) {
      gsap.set(tabbar, { y: -tabbarHeight, opacity: 0 });
      gsap.set(navbarWrapper, { height: navHeight });
    } else {
      gsap.set(tabbar, { y: 0, opacity: 1 });
      gsap.set(navbarWrapper, { height: fullHeight });
    }
  }
