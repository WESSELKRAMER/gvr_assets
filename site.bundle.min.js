...
document.addEventListener("DOMContentLoaded", () => {
  ...
  triggers.forEach((trigger) => {
    trigger.addEventListener("mouseenter", () => {
      ...
      const panel = document.querySelector(`[data_menu_panel="${key}"]`);
      if (!panel || !megaMenu) return;

      panels.forEach((p) => p.classList.remove("is_active"));
      panel.classList.add("is_active");

      megaMenu.style.height = panel.offsetHeight + "px";
    });
  });

  document.querySelector(".navbar")?.addEventListener("mouseleave", () => {
    panels.forEach((p) => p.classList.remove("is_active"));
    if (megaMenu) megaMenu.style.height = "0px";
  });
});
