...
function getActiveNavbar() {
  const params = new URLSearchParams(window.location.search);
  const activeNav = normalizeNavValue(params.get("nav") || "vrije-tijd");

  const navbars = [...document.querySelectorAll(".navbar[data-nav]")];
  if (!navbars.length) return null;

  return navbars.find((navbar) => {
    return normalizeNavValue(navbar.getAttribute("data-nav")) === activeNav;
  }) || null;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".navbar[data-nav]").forEach((nav) => {
    nav.style.display = nav === navbar ? "" : "none";
  });
