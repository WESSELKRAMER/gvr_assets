...
// Nav Collapse
document.addEventListener("DOMContentLoaded", () => {
  initNavbarOverlapScroll();
});

function initNavbarOverlapScroll() {
  const navbarGroups = document.querySelectorAll(".navbar_wrapper");

  navbarGroups.forEach((navbarWrapper) => {
    if (!isActuallyVisible(navbarWrapper)) return;

    const tabbar = navbarWrapper.querySelector(".tabbar_wrapper");
    const nav = navbarWrapper.querySelector(".nav_wrapper");

    if (!tabbar || !nav) return;
    ...
    function collapse() {
      if (isCollapsed || !isActuallyVisible(navbarWrapper)) return;
      isCollapsed = true;
...
function initTabbarDropdowns() {
  const mobileBreakpoint = 991;
  const wrappers = document.querySelectorAll(".tabbar_wrapper");

  wrappers.forEach((wrapper) => {
    if (wrapper.dataset.tabbarInit === "true") return;
    wrapper.dataset.tabbarInit = "true";

    if (!isActuallyVisible(wrapper)) return;

    const navbarRoot = wrapper.closest(".navbar") || document;
    ...
    function closeOtherDropdownsInSameNavbar() {
      const siblingWrappers = navbarRoot.querySelectorAll(".tabbar_wrapper");

      siblingWrappers.forEach((otherWrapper) => {
        if (otherWrapper === wrapper) return;
        const closeFn = otherWrapper._closeTabbarDropdown;
    ...
    function openMenu() {
      if (isAnimating || !isMobile() || !isActuallyVisible(wrapper)) return;

      closeOtherDropdownsInSameNavbar();
    ...
    function closeMenu(force = false) {
      if (((!isOpen && !force) || isAnimating || !isMobile())) return;
    ...
    wrapper._closeTabbarDropdown = closeMenu;
